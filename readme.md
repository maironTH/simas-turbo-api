...
2. Execute os próximos comandos na raiz do repositório, onde está o arquivo `SimasTurbo.csproj`. Instale o .NET 10 SDK e confirme a instalação:
   ```bash
   dotnet --version
   ```
...
6. Restaure as dependências e execute a API:
   ```bash
   dotnet restore
   dotnet run --project SimasTurbo.csproj
   ```
...
Se a aplicação informar que `JwtConfig:Secret` não foi encontrado, execute os comandos a partir da raiz do repositório, onde está o arquivo `SimasTurbo.csproj`:
